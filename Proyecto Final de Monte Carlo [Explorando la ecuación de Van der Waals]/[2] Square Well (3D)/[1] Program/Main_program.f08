...
PROGRAM CODIGO4
    IMPLICIT NONE
        ...
        REAL(KIND=8)                :: ACM, VN, ACEPRATIO, ZIOLD, QSEED, ZINEW, lambda, TS
...
! LECTURA DE DATOS DE ENTRADA ----------------------------------------------------------|
    ...
    WRITE(*,*) 'Establezca el criterio de aceptacion (RATIO)'
        READ (*,*) ACEPRATIO
    WRITE(*,*) 'Introduzca el ancho del pozo cuadrado (SW) (lambda)'
        READ (*,*) lambda
    WRITE(*,*) 'Introduzca el valor de T* (TS)'
        READ (*,*) TS
...
! ESCRIBIR DATOS CAPTURADOS Y CALCULADOS EN PANTALLA -----------------------------------|
    ...
    WRITE(15,*) '   Ancho del pozo cuadrado    lambda = ',  lambda
...
!   Cálculo de la energía de la configuración inicial:
        CALL SUMUP (N, RCUT, BOXL, V, X, Y, Z, lambda, TS)
        VI=V+VLRC
            WRITE(*,*) '------------------------------------------------------------'
...
!   Cálculo de energía de la configuración actual: 
                CALL ENERGY(N, BOXL, XIOLD, YIOLD, ZIOLD, I, RCUT, VOLD, X, Y, Z, lambda, TS)
...
!   Cálculo de energía de la configuración nueva:            
                CALL ENERGY(N, BOXL, XINEW, YINEW, ZINEW, I, RCUT, VNEW, X, Y, Z, lambda, TS)
